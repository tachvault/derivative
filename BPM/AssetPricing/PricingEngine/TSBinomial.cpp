...
namespace derivative
{
	...
	void TSBinomialMethod::rollbackTermstructures()
	{
		...
		for (i = n - 1; i >= 0; i--)
		{
			...
			for (j = nodes[i]->begin(); j != nodes[i]->end(); j++)
			{
				...
				(*j)->ts.reset(new TSLogLinear(T(Range(i, n + 1)), B(Range(0, n - i + 1), jidx++)));
			}
		}
	}
	...
	{
		std::function<double(double, const TermStructure&)> f;
		f = std::bind(std::mem_fn(&TSBermudanInstrument::payoff), &instrument, _1, _2);
		...
		if (i<0) throw std::logic_error("Time line mismatch");
		std::function<double(const TermStructure&)> g;
		g = std::bind(std::mem_fn(&TSBermudanInstrument::payoff), &instrument, 0.0, _1);  // payoff on last exercise date
		apply_payoff(i, g);
		rollback(i, 0, f);
		return result();
	}
