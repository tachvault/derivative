...
namespace derivative
{
	GeometricBrownianMotion::GeometricBrownianMotion(std::vector<std::shared_ptr<BlackScholesAssetAdapter> >& xunderlying)
		: underlying(xunderlying), T(nullptr), timeline_(nullptr), asset_values(xunderlying.size(), 2), time_mapping(nullptr),
	...
	bool GeometricBrownianMotion::set_timeline(const Array<double, 1>& timeline)
	{
		...
		T = std::unique_ptr<Array<double, 1> >(new Array<double, 1>(segments.copy()));
		timeline_ = std::unique_ptr<Array<double, 1> >(new Array<double, 1>(timeline.copy()));
		// map timeline indices to segments
		time_mapping = std::unique_ptr<Array<int, 1> >(new Array<int, 1>(timeline.extent(firstDim)));
		j = 0;
		...
		for (i = 0; i < underlying.size(); i++)
		{
			asset_values(i, 0) = underlying[i]->GetAssetValue()->GetTradePrice();
		}
		return true;
	}
}
