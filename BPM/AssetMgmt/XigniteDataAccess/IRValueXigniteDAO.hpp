...
namespace derivative
{
	...
	class IRValueXigniteDAO : virtual public IDAO,
		             virtual public IObject,
					 virtual public IMake					 
	{
	public:

		enum {TYPEID = CLASS_INTERESTRATEVALUEXIGNITEDAO_TYPE};

		/// constructors.
		/// Constructor with Exemplar for the Creator IRValueXigniteDAO object
		IRValueXigniteDAO (const Exemplar &ex)
			:m_name(TYPEID)
		{
		}
		
		/// Constructor for IMake::Make(..)
		IRValueXigniteDAO(const Name& daoName)
			:m_name(daoName)
		{
		}
		...
		{
			LOG(WARNING) << " IDAO::find(..) not applicable for IRValue entity" << endl;
			throw DataSourceException("IDAO::find(..) not applicable for IRValue entity");
		}
	...
	private:
		...
		Concurrency::task_status  findIRValue(const dd::date& issueDate);
