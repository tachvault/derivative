...
namespace derivative
{
	...
	//GROUP_REGISTER(GARCH);

	GARCH::GARCH(const Exemplar &ex)
		:m_name(TYPEID)
	{
	}

	GARCH::GARCH(const Name& nm)
		: m_name(nm)
	{
	}

	/// construct GARCH from country and process date
	GARCH::GARCH(const Country& cntry, const dd::date& processDate, GARCH::DataSourceType src)
	...
	void GARCH::BuildGARCHFromBond()
	{
		...
		/// get all the ZCB/coupon bonds corresponding to this country
		/// the trade date would the processed date of this GARCH.
	...
	void GARCH::BuildGARCHFromIIBOR()
	{
	...
	void GARCH::FindZCBBondValues(std::vector<std::shared_ptr<IZeroCouponBondValue> >& zcbs)
	{
	...
	void GARCH::FindCouponBondValues(std::vector<std::shared_ptr<IFixedRateBondValue> >& cbonds)
	{
		/// Get all the zero coupon bonds with symbol starting with this country code and
		/// tradedate as this processed date 
	...
	void GARCH::FindLiborRateValues(std::vector<std::shared_ptr<IIBORValue> >& rates)
	{
	...
	void GARCH::FindIRRateValues(std::vector<std::shared_ptr<IIRValue> >& rates)
	{
	...
	std::shared_ptr<GARCH> BuildGARCH(const GARCH::DataSourceType src, const std::string& cntryCode, const dd::date& edate)
	{
		/// register the exemplar here until the bug on visual C++ is fixed.
		GroupRegister GARCHGrp(GARCH::TYPEID, std::make_shared<GARCH>(Exemplar()));

		Name curName = GARCH::ConstructName(cntryCode, edate, src);
		...
		if (obj)
		{
			std::shared_ptr<GARCH> curve = dynamic_pointer_cast<GARCH>(obj);
			return curve;
		}
		...
		std::shared_ptr<GARCH>  GARCH = std::make_shared<GARCH>(cntry, edate, src);
