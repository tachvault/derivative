...
namespace derivative
{
	...
	const std::shared_ptr<IObject> FuturesValueMySQLDAO::find(const Name& nm)
	{
		...
		try
		{
			...
			while (res->next())
			{
				...
				m_futuresVal->SetAccessTime(pt::second_clock::local_time());
	...
	bool FuturesValueMySQLDAO::refresh(shared_ptr<IObject>& obj)
	{
		LOG(INFO) << "FuturesValueMySQLDAO::refresh(..) is called for " << obj->GetName() << endl;
		m_futuresVal = dynamic_pointer_cast<IFuturesValue>(obj);
		...
		dd::date tradeDate;
		dd::date deliveryDate;
		IFuturesValue::GetKeys(obj->GetName(), symbol, tradeDate, deliveryDate);
		Name futuresName(IFutures::TYPEID, std::hash<std::string>()(symbol));

		/// Update the futures value with latest data from MySQL.
		try
		{
