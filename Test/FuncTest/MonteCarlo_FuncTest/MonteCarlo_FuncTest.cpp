...
TEST_F(MonteCarloTest, LoadLibraries) 
{
	...
	/// load RESTDataAccess  component explicity
...
TEST_F(MonteCarloTest, CVTestPower) 
{
	...
	try
	{
		...
		/// setup option parameters/// Get domestic interest rate of the stock
		auto cntry = stockVal->GetStock()->GetCountry().GetCode();
		dd::date today = dd::day_clock::local_day();
		double mat = 1.5;
		double r = PrimaryUtil::FindInterestRate(cntry, mat, IRCurve::YIELD);
		...
		cv_strikes *= K;
		for (int i=0;i<5;i++) cv_values(i) = gasset->option(T(T.extent(firstDim)-1)-T(0),cv_strikes(i),r);
		...
		for (int i=0;i<5;i++) 
		{
			std::shared_ptr<MCEuropeanCall> callpayoff(new MCEuropeanCall(T(0),T(T.extent(firstDim)-1),0,cv_strikes(i)));
			cvlist.push_back(callpayoff); 
		}
		// Test list
...
TEST_F(MonteCarloTest, LSMC) 
{
	...
	try
	{
		...
		double price = stockVal->GetTradePrice();
		stockVal->SetTradePrice(stockVal->GetTradePrice()*1.5);
		std::shared_ptr<BlackScholesAssetAdapter> tmpstock = \
			std::make_shared<BlackScholesAssetAdapter>(stockVal);
		tmpstock ->SetDivYield(0);
		...
		cout << Mput.price(tmp_t,price*1.5) << endl;
		GeometricBrownianMotion gbm(underlying);
		gbm.set_timeline(T);
		ranlib::NormalUnit<double> normalRNG;
...
TEST_F(MonteCarloTest, LSMCMargrabe) 
{
	...
	{
		std::string symbol("AAPL");
		std::shared_ptr<IStockValue> stockValuePtr1 = getStockValue(symbol);
		/// If we are here without any assert failures mean, we got apple stock
		/// object and current apple stock value.
		std::shared_ptr<IStockValue> stockVal1 = dynamic_pointer_cast<IStockValue>(stockValuePtr1);
		double aDivYield = stockVal1->GetDivYield();

		/// Get the name for Google stock (stored in MySQL database)
		std::string symbol2("GOOG");
		std::shared_ptr<IStockValue> stockValuePtr2 = getStockValue(symbol2);
		/// If we are here without any assert failures mean, we got apple stock
		/// object and current apple stock value.
		std::shared_ptr<IStockValue> stockVal2 = dynamic_pointer_cast<IStockValue>(stockValuePtr2);
		double gDivYield = stockVal2->GetDivYield();

		/// Get domestic interest rate of the stocks 
		/// Note that this test is based on assets both have the 
		/// same domestic country
		auto cntry = stockVal1->GetStock()->GetCountry().GetCode();
		dd::date today = dd::day_clock::local_day();
		double rho = 0.0;
		double mat = 1.5;
		double r = PrimaryUtil::FindInterestRate(cntry, mat, IRCurve::YIELD);
		double K = 1.0;
		K *= stockVal1->GetTradePrice()/stockVal2->GetTradePrice();
		int N = 10;
		size_t minpaths = 100;
		size_t train = 100;
		int degree = 2;
		...
		bool include_put = false;
		int numeraire_index = -1;
		stockVal1->SetDivYield(0);
		stockVal2->SetDivYield(0);
		...
		/// BlackScholes object for apple stock
		...
		auto applePrice = stockVal1->GetTradePrice();
		stockVal1->SetTradePrice(1.5*applePrice);
		auto googlePrice = stockVal2->GetTradePrice();
		stockVal2->SetTradePrice(1.6*applePrice);

		/// BlackScholes object for apple stock
		vol1.reset(new ConstVol(sgm_1));
		std::shared_ptr<BlackScholesAssetAdapter> tmpstock1 = \
			std::make_shared<BlackScholesAssetAdapter>(stockVal1,vol1);
		tmpstock1->SetDivYield(0);

		vol2.reset(new ConstVol(sgm_2));
		std::shared_ptr<BlackScholesAssetAdapter> tmpstock2 = \
			std::make_shared<BlackScholesAssetAdapter>(stockVal2,vol2);
		tmpstock2->SetDivYield(0);

		stockVal1->SetTradePrice(applePrice);
		stockVal2->SetTradePrice(applePrice);
		...
		S_history(0,N/2) = applePrice*1.5;
		S_history(1,N/2) = googlePrice*1.6;
		...
		cout << "Dividend 1: " << stockVal1->GetDivYield() << "   Dividend 2: " << stockVal2->GetDivYield() << endl;
...
TEST_F(MonteCarloTest, LSMCPath) 
{
	...
	try
	{
		...
		/// BlackScholes object for google stock
		auto price = stockVal->GetTradePrice();
		stockVal->SetTradePrice(price*1.5);
		std::shared_ptr<BlackScholesAssetAdapter> tmpstock = \
			std::make_shared<BlackScholesAssetAdapter>(stockVal);
		tmpstock->SetDivYield(0);
		stockVal->SetTradePrice(price);
		...
		exotics::StandardOption tmpMput(tmpstock,tmp_t,mat,K,ts,-1);
		cout << "Testing: " << tmpMput.price() << " == ";
		cout << Mput.price(tmp_t,stockVal->GetTradePrice()*1.5) << endl;
		GeometricBrownianMotion gbm(underlying);
		gbm.set_timeline(T);
		ranlib::NormalUnit<double> normalRNG;
		RandomArray<ranlib::NormalUnit<double>,double> random_container(normalRNG,gbm.factors(),gbm.number_of_steps()); 
...
TEST_F(MonteCarloTest, MCDownAndOut) 
{
	...
	try
	{
		...
		dd::date today = dd::day_clock::local_day();
		double maturity = 1.5;
		double r = PrimaryUtil::FindInterestRate(cntry, maturity, IRCurve::YIELD);
		double K = 1.0;
		K *= stockVal->GetTradePrice();
		double call_strike = K;
		int N = 10;
		...
		double CFiput = stock->option(T(N/2),K,r,-1);
		cout << "Closed form vanilla call: " << CFcall << endl;
		cout << "Closed form vanilla put: " << CFput << endl;
		cout << "Closed form intermediate maturity put: " << CFiput << endl;
...
TEST_F(MonteCarloTest, MCGenericArrayTest) 
{
	...
	try 
	{
		...
		objective_class obj(stockVal->GetTradePrice(),mat,r,sgm,K);
...
TEST_F(MonteCarloTest, MCGenericTest) 
{
	std::cout << "==================================================================" << std::endl;
	...
	try 
	{
		...
		ranlib::NormalUnit<double> normal_RNG;
		objective_class obj(stockVal->GetTradePrice(),mat,r,sgm,K);
		std::function<double (double)> func = std::bind(&objective_class::func2,&obj,std::placeholders::_1);
...
TEST_F(MonteCarloTest, MCMargrabe) 
{
	...
	try 
	{
		/// Get the name for Apple stock (stored in MySQL database)
		std::string symbol("AAPL");
		std::shared_ptr<IStockValue> stockValuePtr = getStockValue(symbol);
		/// If we are here without any assert failures mean, we got apple stock
		/// object and current apple stock value.
		...
		/// now construct the BlackScholesAdapter from the stock value.
		std::shared_ptr<BlackScholesAssetAdapter> stock2 = \
			std::make_shared<BlackScholesAssetAdapter>(stockVal2,std::move(vol2));
		underlying.push_back(stock2);
...
TEST_F(MonteCarloTest, QRCVTest) 
{
	...
	try 
	{
		...
		/// now construct the BlackScholesAdapter from the stock value.
		std::shared_ptr<BlackScholesAssetAdapter> gasset = \
			std::make_shared<BlackScholesAssetAdapter>(stockVal,std::move(gvol));
