...
namespace derivative
{
	...
	{
		if (const char* env_p = std::getenv("DATA_PATH"))
		{
			std::string dir_file = std::string(env_p) + std::string("\\") + direction_number_file;
			initialise(dir_file.c_str());
		}
		else
		{
			throw std::runtime_error("Sobol directional file path not found");
		}
	}
