...
namespace derivative
{
	...
	double interfaceCLAPACK::PositiveSymmetricMatrixDeterminant(const Array<double,2>& A)
	{
		int i;
		Array<double,2> L(A.extent(blitz::firstDim),A.extent(blitz::secondDim));
		interfaceCLAPACK::Cholesky(A,L,'L');
		double det = 1.0;
	...
	void interfaceCLAPACK::Cholesky(const Array<double,2>& A,Array<double,2>& triangular,char LorU)
	{
		int i,j;
		long int n = A.rows();
	...
	/// Determinant of a real symmetric positive definite matrix.
	void interfaceCLAPACK::PositiveSymmetricMatrixInverse(const Array<double,2>& A,Array<double,2>& inverseA)
	{
		int i,j;
		long int n = A.rows();
		if (n!=A.columns()) throw(std::logic_error("Array must be square"));
	...
	void interfaceCLAPACK::MoorePenroseInverse(const Array<double,2>& A,Array<double,2>& inverseA,double eps)
	{
		int m = A.rows();
		int n = A.columns();
	...
	void interfaceCLAPACK::SingularValueDecomposition(const Array<double,2>& A,Array<double,2>& U,Array<double,1>& sigma,Array<double,2>& V)
	{
		int i,j;
		long int m = A.rows();
	...
	void interfaceCLAPACK::SolveLinear(const Array<double,2>& A,Array<double,2>& X,const Array<double,2>& B)
	{
		int i,j;
		long int n = A.rows();
		if (n!=A.columns()) throw(std::logic_error("Array must be square"));
	...
	void interfaceCLAPACK::SymmetricEigenvalueProblem(
	...
	void interfaceCLAPACK::SolveTridiagonal(const Array<double,2>& A,Array<double,2>& X,const Array<double,2>& B)
	{
		int i,j;
		long int n = A.rows();
		if (n!=A.columns()) throw(std::logic_error("Array must be square"));
	...
	/// Solve system of linear equations A X = B using CLAPACK routines, where A is a triagonal matrix.
	void interfaceCLAPACK::SolveTridiagonalSparse(const Array<double,2>& A,Array<double,2>& X,const Array<double,2>& B)
	{
		int i,j;
		long int n = A.rows();
