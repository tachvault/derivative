...
{
	void ExchangeRateAverageOptJSONVisitor::Visit(const std::shared_ptr<IMessage>& message, json::value& out)
	{
		json::value reqObj;
		json::value resObj;
		json::value greekObj;

		std::shared_ptr<ExchangeRateAverageOptMessage> msg;
		try
		{
			/// if we are here then the message should be of type ExchangeRateAverageOptMessage.
			msg = dynamic_pointer_cast<ExchangeRateAverageOptMessage>(message);
		}
		...
		reqObj[L"option"] = (msg->GetRequest().option == ExchangeRateAverageOptMessage::CALL) ? json::value::string(U("call")) : json::value::string(U("put"));
		if (msg->GetRequest().averageType == ExchangeRateAverageOptMessage::FIXED_STRIKE)
		{
			reqObj[L"Strike type"] = (msg->GetRequest().averageType == ExchangeRateAverageOptMessage::FIXED_STRIKE) ? json::value::string(U("Fixed strike")) : json::value::string(U("Floating strike"));
		}
		if (msg->GetRequest().averageType == ExchangeRateAverageOptMessage::FIXED_STRIKE) reqObj[L"strike"] = json::value::number(msg->GetRequest().strike);
