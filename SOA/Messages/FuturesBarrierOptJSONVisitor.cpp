...
{
	void FuturesBarrierOptJSONVisitor::Visit(const std::shared_ptr<IMessage>& message, json::value& out)
	{
		json::value reqObj;
		json::value resObj;
		json::value greekObj;

		std::shared_ptr<FuturesBarrierOptMessage> msg;
		try
		{
			/// if we are here then the message should be of type FuturesBarrierOptMessage.
			msg = dynamic_pointer_cast<FuturesBarrierOptMessage>(message);
		}
		...
		reqObj[L"option"] = (msg->GetRequest().option == FuturesBarrierOptMessage::CALL) ? json::value::string(U("call")) : json::value::string(U("put"));
		if (msg->GetRequest().barrierType == FuturesBarrierOptMessage::KDI)
		{
			reqObj[L"Barrier option type"] = json::value::string(U("Knock down-and-in"));
		}
		else if (msg->GetRequest().barrierType == FuturesBarrierOptMessage::KDO)
		{
			reqObj[L"Barrier option type"] = json::value::string(U("Knock down-and-out"));
		}
		else if (msg->GetRequest().barrierType == FuturesBarrierOptMessage::KUI)
		{
			reqObj[L"Barrier option type"] = json::value::string(U("Knock up-and-in"));
		}
		else if (msg->GetRequest().barrierType == FuturesBarrierOptMessage::KUO)
		{
